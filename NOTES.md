...
## Final result (Phase 5/6, 56 runs)
- 9/56 recovered within the 10% budget. Pareto frontier: quantize_1_distill (32×, recfrac
  0.085), kmeans_2_distill (16×, 0.060), kmeans_4_distill (8×, 0.005), kmeans_6 (5×, 0.005),
  quantize_8 (4×, 0.005). Quantization & k-means win; all pruning + low-rank DNR; distillation
  extends recovery to 1-bit weights. Full write-up in RESULTS.md.
- Report published (user-approved) at https://jacobcd52.github.io/weight-compression-recovery/
  (repo made public + GitHub Pages from /docs; regenerated by `python -m src.report`).
