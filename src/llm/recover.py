...
def main():
    ...
    # Target must be measured on the SAME eval config used for crossing detection, else a small
    # subset bias makes recovery look harder than it is (the ResNet subset-eval bug). So compute
    # the victim's loss on this exact subset rather than reusing the full-eval target.
    vmodel = _fresh_from(victim_sd, None, device)
    target = eval_loss(vmodel, eb, device, max_batches=args.eval_batches)
    del vmodel
    if device.type == "cuda":
    ...
    print(f"[seedfrontier] target={target:.4f} (eval_batches={args.eval_batches}, "
          f"full-eval victim={summary['target_loss']:.4f}) denom={denom} budget={budget_steps} "
          f"fp32={fp32_bytes/1e6:.1f}MB lr_grid={args.lr_grid}", flush=True)
    ...
    json.dump(dict(target=target, target_full_eval=summary["target_loss"],
                   eval_batches=args.eval_batches, denom=denom, budget_steps=budget_steps,
                   budget_fraction=args.budget_fraction, fp32_bytes=fp32_bytes,
                   lr_grid=args.lr_grid, results=results),
