...
def make_plot(df, out_dir="figures", budget=0.10):
    ...
    for t in techniques:
        ...
        c = colors[t]
        for distill, marker, label_sfx in [(False, "o", ""), (True, "^", " (distill)")]:
            ...
            if not d.empty:
                ax.scatter(d["compression_ratio"], d["recovery_fraction"],
                           ...
                           linewidths=0.5, label=f"{t}{label_sfx}", zorder=3)
    ...
    ax.text(ax.get_xlim()[0], budget, "  budget cap / DNR", va="bottom",
    ...
    ax.set_title("Weight-compression recovery: compression vs. retraining cost\n"
                 "ResNet-20 / CIFAR-10 (open markers = DNR within 10% budget)")
